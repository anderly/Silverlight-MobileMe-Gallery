...
namespace Gallery.Classes
{
	...
	{
		private const string ALBUM_SUB_TITLE_FORMAT = "{0} photos";
		...
		public string SubTitle
		{
			get
			{
				...
				if (this.Type == GalleryItemType.Album)
				{
					subTitle = string.Format(ALBUM_SUB_TITLE_FORMAT, ((Album)this).PhotoCount);
				}
				else
				{
		...
		} //end public GalleryItem()

		public GalleryItem(record record)
		{
			this.Guid = record.guid;
			this.UserItemGuid = record.userItemGuid;
			this.Title = record.title;
			this.Type = (GalleryItemType)Enum.Parse(typeof(GalleryItemType), record.type, true);
			this.Url = record.url;
			this.Updated = record.updated;
			this.SortOrder = record.sortOrder;
			this.ShowCaptions = record.showCaptions;
			this.AccessLogin = record.accessLogin;
			this.ScrubSpritePath = string.Format("{0}/{1}", this.Url, record.scrubSpritePath);
			this.ScrubSpriteFrameCount = record.scrubSpriteFrameCount;
			this.ScrubSpriteKeyFrameIndex = record.scrubSpriteKeyFrameIndex;
			this.ScrubSpriteFrameHeight = record.scrubSpriteFrameHeight;
			this.ScrubSpriteFrameWidth = record.scrubSpriteFrameWidth;
			this.ViewIdentifier = record.viewIdentifier;
			this.VersionInfo = record.versionInfo;
			this.UserHidden = record.userHidden;
			this.Download = record.download;

		} //end public GalleryItem(record record)
